...
function openPathInOs(filePath: string): void {
  const command =
    process.platform === 'win32'
      ? `start "" ${quoteShellPath(filePath)}`
      : process.platform === 'darwin'
        ? `open ${quoteShellPath(filePath)}`
        : `xdg-open ${quoteShellPath(filePath)}`
...
export class ArtifactService {
  ...
  collectArtifactsForRun(runId: string, runDir: string): void {
    ...
    if (updates.length > 0) {
      params.push(runId)
      this.db.prepare(`UPDATE runs SET ${updates.join(', ')} WHERE id = ?`).run(...params)
    }
  }

  openArtifact(filePath: string): void {
    openPathInOs(filePath)
  }

  openReport(reportPath: string): void {
    openPathInOs(reportPath)
  }
}
