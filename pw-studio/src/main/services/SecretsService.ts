...
export class SecretsService {
  ...
  async checkAvailability(): Promise<boolean> {
    try {
      const keytar = await this.getKeytar()
      // Test by trying to find a non-existent password
  ...
  private async getKeytar(): Promise<typeof import('keytar')> {
    try {
      ...
      throw new SecretsUnavailableError(
        'Keychain not available. On Windows, ensure Credential Manager is running. ' +
        'The keytar native module may need to be rebuilt for your Electron version.'
      )
    }
  }
}
