...
function checkNode(): Promise<HealthItem> {
  return new Promise((resolve) => {
    execFile('node', ['--version'], { timeout: 10_000 }, (err, stdout) => {
      if (err) {
        resolve({ check: 'node', status: 'error', message: 'Node.js not found', actionHint: 'Install Node.js 18+' })
        return
      }
      ...
      if (major < 18) {
        resolve({
          check: 'node',
          status: 'error',
          message: `Node.js ${version} found, minimum 18.x required`,
          value: version,
          actionHint: 'Upgrade Node.js to 18+',
        })
      } else {
